...
const rules = {
  title: [
    ...
    (v?: string) => (v && v.length <= 100) || 'Portfolio Type Title must be 100 characters or less',
  ],
  slug: [
    (v: string) => !!v || 'Portfolio Type Slug is required',
    (v?: string) => (v && v.length <= 70) || 'Portfolio Type Slug must be 70 characters or less',
  ],
};
...
const submit = async (dialog: Ref) => {
  ...
  if (valid) {
    ...
    const routeName = type ? 'portfolio-type.update' : 'portfolio-type.store';
